...
reg         flash_cs;
reg  [21:0] word_count;     // [21:0] statt [31:0] — max. Wert 22'h40001  → -10 FF
reg  [4:0]  state;
...
reg         ks_changed;     // 1-Takt-Puls, im Haupt-Block verwaltet       → -1 FF
...
always_ff @(posedge clk_85m) begin
    restart_rom_copy <= 1'b0;
        ...
        if ((start_rom_copy || restart_rom_copy || ks_changed || state == 5'd23) && !rom_done) begin
            flash_cs  <= 1'b1;
