...
always_ff @(posedge clk_85m or negedge pll_lock) begin
    if (!pll_lock) begin
        start_rom_copy <= 1'b0;
        mem_ready_D    <= 1'b0;
    end else begin
        mem_ready_D    <= mem_ready;
        start_rom_copy <= mem_ready && !mem_ready_D; // rising-edge pulse
    end
end
...
reg         flash_cs;
reg  [21:0] word_count;     // [21:0] statt [31:0] — max. Wert ist 22'h40001  → -10 FF
reg  [4:0]  state;
...
reg         flash_ram_write;
reg  [5:0]  flash_cnt;
reg  [1:0]  osd_kickstartD; // [1:0] statt [2:0] — nur 2 Bits relevant           → -1 FF

// ---- KS-change detection (sauberer 1-Takt-Puls, kein Latch-Risiko) -----
// Ersetzt restart_rom_copy                                                         → -1 FF
reg         ks_changed;
always_ff @(posedge clk_85m)
    ks_changed <= (osd_kickstart[1:0] != osd_kickstartD);

// ---- Main state machine -------------------------------------------------
always_ff @(posedge clk_85m) begin
    if (!mem_ready) begin
        osd_kickstartD  <= osd_kickstart[1:0];
        ...
        if ((start_rom_copy || ks_changed || state == 5'd23) && !rom_done) begin
            flash_cs  <= 1'b1;
            ...
            if (flash_cnt == 6'd1) begin
                ...
                if ((flash_addr == 22'h3800aa || flash_addr == 22'h3a00aa) &&
                     flash_dout == 16'h6678)
                    flash_doutD <= flash_dout & 16'hf0ff; // bne.b → bra.b
                else
